...
class Video_Analyzer:
    ...
    def initial_processing_for_openpose(self):
        self.offset = preprocess.calculate_offset(self.test_path, self.ref_path)
        preprocess.get_audio_file(self.test_path, self.offset, os.path.join(self.base_path, self.name, "{}_audio.mp3".format(self.name)))

        self.duration = preprocess.get_duration(self.test_path, self.ref_path, self.offset)
        
        preprocess.trim_start_of_video(self.ref_path, self.offset, self.duration)


    def keypoint_processing_for_openpose(self, folder_name1, folder_name2, num_frames):
        rel_no_roms_accs, rel_roms_accs, abs_accs = process_body25.process_openpose_frames(self.name, folder_name1, folder_name2, num_frames, self.base_path, self.fps)
        target_file_name = self.name+"_rel_no_roms_accs_op.npy"
        np.save(target_file_name, rel_no_roms_accs)
        target_file_name = self.name+"_rel_roms_accs_op.npy"
        np.save(target_file_name, rel_roms_accs)
        target_file_name = self.name+"_abs_accs_op.npy"
        np.save(target_file_name, abs_accs)
    ...
    def get_feedback_report(self, accs_file_name, fps):
        ...
        text_file = open("{}_report".format(self.name), "w")
...
def main():
    ...
    test_analyzer.get_feedback_report(name+"_rel_no_roms_accs_sh.npy")
